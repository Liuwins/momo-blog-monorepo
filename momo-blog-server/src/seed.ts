...
async function seed() {
  const seedUsername = process.env.SEED_ADMIN_USERNAME || process.env.ADMIN_USERNAME || 'admin';
  const seedPassword = process.env.SEED_ADMIN_PASSWORD;
  if (!seedPassword || seedPassword.length < 6) {
    throw new Error('SEED_ADMIN_PASSWORD 未设置或长度不足（至少 6 个字符）。该变量仅用于本地演示数据初始化。');
  }
  ...
  const user = await userRepo.save(
    userRepo.create({
      ...
      nickname: '博主',
      avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(seedUsername)}`,
      signature: '记录生活，分享美好',
    }),
  );
  ...
  console.log(`User: ${seedUsername}（密码来自 SEED_ADMIN_PASSWORD，未在日志中输出）`);
