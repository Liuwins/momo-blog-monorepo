...
状态：2026-08-21，根据当前源码和本地验证更新；详细执行顺序见 [`docs/OPTIMIZATION-PLAN.md`](docs/OPTIMIZATION-PLAN.md)。本轮优化仍有本地提交待公开推送，真实服务器验收不以本地结果替代。
...
- 阶段 1 代码已补齐动态配乐、分页边界、数组元素校验、生产配置保护、Open Graph 域名参数化和 Compose 后端暴露面收敛；当前代码已在本地提交，公开推送前仍需完成最终审查；
- 阶段 2 已建立前后端 Vitest 基线；前端覆盖全局音乐状态和发布页路由守卫，后端覆盖 Auth、Posts、Comments、Upload、WebSocket、Migration 和 DTO 核心规则；CI 已加入公开仓库敏感/生成文件扫描、运维脚本语法检查和高危依赖门禁，真实 CI 运行结果仍待补齐；
- 真实生产域名、WebSocket、上传和备份恢复尚未完成现场验证；本地临时 Docker 后端已完成 migration 与 `/api/health` 验证。

### 阶段 2 本轮进度

- 新增前端 Vitest 配置和 5 条回归测试；
- 新增后端 Vitest 配置、reflect-metadata 测试初始化和 27 条核心业务回归测试，覆盖 Auth、Posts、Comments、Upload、WebSocket、Migration、Health 和 DTO；
...
- 后端回归测试增至 27 条，类型检查和构建通过；已加入脱敏内存数据集查询基准和媒体报告/隔离清理流程，目标服务器真实延迟仍待现场测量。
...
6. **真实环境验收**：在目标服务器验证 GitHub CI、迁移、HTTPS、WebSocket、上传、备份恢复和真实域名；本地构建不替代现场验收。
7. **可选增强**：待前述项目稳定后，再做 PWA 离线能力、动态编辑历史、通知去重和更严格媒体内容校验。
