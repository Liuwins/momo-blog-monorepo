...
- 后端回归测试增至 27 条，类型检查和构建通过；真实数据集查询计划和响应基准仍待补齐。
...
3. **查询性能基准（进行中）**：使用不落盘的脱敏内存数据集，记录查询次数、`EXPLAIN QUERY PLAN`、响应时间和内存，形成逐条查询与批量查询对比；目标服务器真实延迟仍需现场测量。
4. **媒体资源治理（进行中）**：增加上传统计、孤立文件报表和可回滚清理流程；媒体报告脚本已加入，保持默认只报告，生产清理需显式开关。
5. **移动端与可访问性（进行中）**：已处理键盘焦点、ARIA、触摸目标、底部安全区和 reduced-motion，并完成 390×844 本地首页/详情/登录/发布守卫冒烟；真实设备和通知/上传现场仍待复测。
