...
- 所有帖子、评论、用户、关注、点赞、历史版本和 Open Graph 路径 ID 统一使用正整数管道；非法 ID 在控制器层返回 400，不再把 `NaN` 或非正数传入数据库。
- 对动态、头像、图片封面、视频封面和背景音乐引用限制为安全的同源路径或 `http(s)` URL，拒绝危险协议、路径穿越、控制字符和 `simple-array` 分隔符。
...
- 已完成前端 Vitest 配置；
- 最新公开快照提交 `ea17ae2` 已补充动态标签首尾空格规范化和空标签拒绝；对应 GitHub Actions 运行 `32516365231` 的前端、后端和仓库卫生门禁全部通过。
...
- 后端已补 Auth、Posts、Comments、Upload、WebSocket、Migration、Health、DTO、环境校验、媒体引用、路径 ID、评论关联 ID、游客标识、标签边界与规范化、关键词通配符校验、通知去重回归测试（55 条）；本地与远程测试、类型检查和构建均已通过。
...
- 已完善媒体清理脚本的引用扫描，覆盖图片、视频、动态配乐、头像、图片封面、视频封面和背景音乐；保持默认只报告。
...
阶段 4 当前进度：批量关联查询、索引 migration、点赞/评论事务一致性、动态流统一序列化（匿名摘要先过滤不可见评论）、关注关系幂等、关键词/完整标签查询、游客标识边界、标签规范化、查询基准和媒体报告/隔离清理已完成，并通过 55 条后端测试、类型检查和构建；目标服务器真实延迟仍待现场测量。
...
| 7 | NestJS 11 运行时与依赖升级 | 依赖/运行时 | 核心包、Config、JWT、Passport、TypeORM、WebSockets、CLI 和 Schematics 升级；后端 55 条测试、typecheck/build、生产审计和隔离 Compose 通过 | 已完成（本地与 CI） |
...
阶段 6 本地验证：前端 lint、21 条 Vitest、build；后端 55 条 Vitest、typecheck、build；两端高危级别审计通过；隔离端口完整 Compose 和 390×844 浏览器冒烟通过，不能替代目标服务器验收。

本轮封面视频增强（2026-08-22）：首页和个人主页可上传 MP4/WebM 封面视频，后端新增 `users.bgVideo` 字段和可回放 migration；浏览器端使用静音、循环、`playsinline` 播放，视频加载失败时回退到图片或主题背景。封面视频沿用管理员 JWT 上传权限和 50 MB 限制；媒体报告与隔离清理已同步扫描 `bgVideo`。当前只完成本地自动化和本地页面验证，真实设备解码、目标服务器上传和 CDN/代理大文件行为仍待阶段 8 现场验收。
...
- 后端 55 条测试、typecheck、build 和官方 npm 生产依赖审计均通过；新增标签首尾空格规范化与空标签拒绝回归；
