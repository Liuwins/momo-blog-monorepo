...
- 本地自动化验证：前端测试 21/21、lint 0 error/0 warning、build 通过；后端（NestJS 11）测试 55/55、typecheck/build 通过，新增标签首尾空格规范化与空标签拒绝回归；仓库卫生脚本、Compose 插值和 Bash 脚本语法通过；前端及后端官方 npm 生产依赖审计均为 0。公开快照 `ea17ae2` 对应 GitHub Actions `32516365231` 已通过全部门禁。
